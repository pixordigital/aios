...
AUTOMATION_TEMPLATES = {
    ...
    "clinica": {"name": "Clínica — Confirmação consulta", "description": "Webhook agenda → WhatsApp confirmação + lembrete D-1 (case 30% menos falta)", "icon": "🏥", "trigger": {"type": "webhook", "name": "Nova consulta"}, "nodes": [{"label": "Confirmar WhatsApp", "tool_name": "transcribe", "tool_args": {"media_id": "{{json.text}}"}, "output_key": "conf"}]},
    ...
    "ecommerce": {"name": "E-commerce — Recuperação carrinho", "description": "Abandono → WhatsApp cupom 10% → pedido (case R$18k/mês)", "icon": "🛒", "trigger": {"type": "webhook", "name": "Carrinho abandonado"}, "nodes": [{"label": "Enviar cupom", "tool_name": "http_request", "tool_args": {"url": "https://httpbin.org/post", "method": "POST", "body": {"text": "Seu carrinho expira! Cupom 10%: VOLTA10 {{json.link}}"}}, "output_key": "cupom"}]},
}
