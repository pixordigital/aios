...
AUTOMATION_TEMPLATES = {
    ...
    "clinica": {"name": "Clínica — Confirmação consulta", "description": "Webhook agenda → WhatsApp confirmação + lembrete D-1 (case 30% menos falta)", "icon": "🏥", "trigger": {"type": "webhook", "name": "Nova consulta"}, "nodes": [{"label": "Confirmar WhatsApp", "tool_name": "http_request", "tool_args": {"url": "http://evolution:8080/message/sendText/{{env.EVO_INSTANCE}}", "method": "POST", "headers": {"apikey": "{{env.EVO_API_KEY}}"}, "body": {"number": "{{json.json.phone}}", "textMessage": {"text": "Olá {{json.json.name}}! Confirmamos sua consulta amanhã às {{json.json.hora}}. Responda SIM para confirmar."}}}, "output_key": "conf"}]},
    "imobiliaria": {"name": "Imobiliária — Lead qualificado", "description": "Lead portal → qualifica BANT → agenda visita (case +22% conversão)", "icon": "🏠", "trigger": {"type": "webhook", "name": "Lead imóvel"}, "nodes": [{"label": "Qualificar", "tool_name": "lead_score", "tool_args": {"lead": {"name": "{{json.name}}"}}, "output_key": "score"}, {"label": "Agendar", "tool_name": "http_request", "tool_args": {"url": "https://httpbin.org/post", "method": "POST", "body": {"lead": "{{json.name}}"}}, "depends_on": ["__prev__"], "output_key": "visita"}, {"label": "Notificar WhatsApp", "tool_name": "http_request", "tool_args": {"url": "http://evolution:8080/message/sendText/{{env.EVO_INSTANCE}}", "method": "POST", "headers": {"apikey": "{{env.EVO_API_KEY}}"}, "body": {"number": "{{json.json.phone}}", "textMessage": {"text": "Olá {{json.json.name}}, visita agendada! Qual melhor horário?"}}}, "depends_on": ["__prev__"], "output_key": "zap"}]},
    "ecommerce": {"name": "E-commerce — Recuperação carrinho", "description": "Abandono → WhatsApp cupom 10% → pedido (case R$18k/mês)", "icon": "🛒", "trigger": {"type": "webhook", "name": "Carrinho abandonado"}, "nodes": [{"label": "Enviar cupom", "tool_name": "http_request", "tool_args": {"url": "http://evolution:8080/message/sendText/{{env.EVO_INSTANCE}}", "method": "POST", "headers": {"apikey": "{{env.EVO_API_KEY}}"}, "body": {"number": "{{json.json.phone}}", "textMessage": {"text": "Seu carrinho expira! Cupom 10%: VOLTA10 {{json.json.link}} – Finalize agora 👉 {{json.json.checkout_url}}"}}}, "output_key": "cupom"}]},
    "whatsapp_boas_vindas": {"name": "WhatsApp — Boas-vindas", "description": "Novo contato WhatsApp → resposta automática + tag CRM (ideal pra quem começa)", "icon": "📱", "trigger": {"type": "webhook", "name": "Mensagem WhatsApp"}, "nodes": [{"label": "Resposta boas-vindas", "tool_name": "http_request", "tool_args": {"url": "http://evolution:8080/message/sendText/{{env.EVO_INSTANCE}}", "method": "POST", "headers": {"apikey": "{{env.EVO_API_KEY}}"}, "body": {"number": "{{json.json.phone|json.from_number}}", "textMessage": {"text": "Olá! 👋 Bem-vindo à {{env.EMPRESA_NOME}}. Como posso ajudar hoje? Digite 1-Vendas 2-Suporte"}}}, "output_key": "boasvindas"}]},
    "whatsapp_qualificacao": {"name": "WhatsApp — Qualificação SDR", "description": "Lead WhatsApp → qualifica com IA → distribui pra closer (BANT)", "icon": "🎯", "trigger": {"type": "webhook", "name": "Lead WhatsApp"}, "nodes": [{"label": "Qualificar com IA", "tool_name": "code", "tool_args": {"code": "msg=json.get('body','')\nscore= 90 if 'preço' in msg.lower() or 'valor' in msg.lower() else 50\noutput={'score':score, 'quente': score>70}"}, "output_key": "quali"}, {"label": "Notificar closer se quente", "tool_name": "http_request", "tool_args": {"url": "http://evolution:8080/message/sendText/{{env.EVO_INSTANCE}}", "method": "POST", "headers": {"apikey": "{{env.EVO_API_KEY}}"}, "body": {"number": "{{env.CLOSER_PHONE}}", "textMessage": {"text": "Lead quente {{json.json.phone}} score {{outputs.quali.score}} — {{json.body}}"}}}, "depends_on": ["__prev__"], "condition": "outputs['quali']['quente']", "output_key": "alerta"}]},
    "whatsapp_handover": {"name": "WhatsApp — Handover humano", "description": "Detecta 'falar com humano' → pausa bot e notifica atendente (evita frustração)", "icon": "🙋", "trigger": {"type": "webhook", "name": "Pedido humano"}, "nodes": [{"label": "Detectar handover", "tool_name": "if_branch", "tool_args": {"condition": "'humano' in json.get('body','').lower() or 'atendente' in json.get('body','').lower()", "input": {"msg": "{{json.body}}"}}, "output_key": "precisa_humano"}, {"label": "Pausar bot + notificar", "tool_name": "http_request", "tool_args": {"url": "http://evolution:8080/message/sendText/{{env.EVO_INSTANCE}}", "method": "POST", "headers": {"apikey": "{{env.EVO_API_KEY}}"}, "body": {"number": "{{json.json.phone}}", "textMessage": {"text": "Te conectando com um humano em instantes 🙏 Aguarde..."}}}, "depends_on": ["__prev__"], "condition": "outputs['precisa_humano']", "output_key": "handover"}]},
    "whatsapp_template": {"name": "WhatsApp — Disparo com template Meta", "description": "Fora da janela 24h → usa template aprovado Meta (evita 131047)", "icon": "📋", "trigger": {"type": "webhook", "name": "Disparo template"}, "nodes": [{"label": "Enviar template", "tool_name": "http_request", "tool_args": {"url": "http://evolution:8080/message/sendWhatsApp/{{env.EVO_INSTANCE}}", "method": "POST", "headers": {"apikey": "{{env.EVO_API_KEY}}"}, "body": {"number": "{{json.json.phone}}", "type": "template", "template": {"name": "{{json.template|default('hello_world')}}", "language": {"code": "pt_BR"}, "components": [{"type": "body", "parameters": [{"type": "text", "text": "{{json.json.name}}"}]}]}}}, "output_key": "tpl"}]},
    "crm_followup_100": {"name": "CRM — Follow-up 100% IA + HITL", "description": "Cron 4h: fila CRM → WhatsApp follow-up automático. >R$5k vai pra aprovação (HITL)", "icon": "🔄", "trigger": {"type": "cron", "cron": "0 */4 * * *", "name": "Follow-up 4h"}, "nodes": [{"label": "Buscar fila CRM", "tool_name": "http_request", "tool_args": {"url": "{{env.AIOS_URL}}/api/crm/queue", "method": "GET", "headers": {"Authorization": "Bearer {{env.CRM_TOKEN}}"}}, "output_key": "fila"}, {"label": "Enviar follow-up", "tool_name": "http_request", "tool_args": {"url": "http://evolution:8080/message/sendText/{{env.EVO_INSTANCE}}", "method": "POST", "headers": {"apikey": "{{env.EVO_API_KEY}}"}, "body": {"number": "{{outputs.fila[0].phone}}", "textMessage": {"text": "Olá {{outputs.fila[0].name}}! Passando pra follow-up do seu interesse. Posso tirar dúvidas?"}}}, "depends_on": ["__prev__"], "output_key": "followup"}]},
}
