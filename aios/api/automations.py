...
AUTOMATION_TEMPLATES = {
    ...
    "clinica": {"name": "Clínica — Confirmação consulta", "description": "Webhook agenda → WhatsApp confirmação + lembrete D-1 (case 30% menos falta)", "icon": "🏥", "trigger": {"type": "webhook", "name": "Nova consulta"}, "nodes": [{"label": "Confirmar WhatsApp", "tool_name": "http_request", "tool_args": {"url": "https://api.evolution/send", "method": "POST", "body": {"text": "Olá {{json.name}}, confirme consulta {{json.data}}?"}}, "output_key": "conf"}]},
    "imobiliaria": {"name": "Imobiliária — Lead qualificado", "description": "Lead portal → qualifica BANT → agenda visita (case +22% conversão)", "icon": "🏠", "trigger": {"type": "webhook", "name": "Lead imóvel"}, "nodes": [{"label": "Qualificar", "tool_name": "lead_score", "tool_args": {"lead": {"name": "{{json.name}}"}}, "output_key": "score"}, {"label": "Agendar", "tool_name": "http_request", "tool_args": {"url": "https://api.example.com/visita", "method": "POST", "body": {"lead": "{{json.name}}"}}, "depends_on": ["__prev__"], "output_key": "visita"}]},
    "ecommerce": {"name": "E-commerce — Recuperação carrinho", "description": "Abandono → WhatsApp cupom 10% → pedido (case R$18k/mês)", "icon": "🛒", "trigger": {"type": "webhook", "name": "Carrinho abandonado"}, "nodes": [{"label": "Enviar cupom", "tool_name": "http_request", "tool_args": {"url": "https://api.evolution/send", "method": "POST", "body": {"text": "Seu carrinho expira! Cupom 10%: VOLTA10 {{json.link}}"}}, "output_key": "cupom"}]},
}
