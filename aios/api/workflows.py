...
async def list_workflows(
    ...
    total = (await db.execute(
        select(__import__('sqlalchemy').func.count(Workflow.id)).where(Workflow.org_id == org_id)
    )).scalar()
...
async def add_node(
    ...
    if body.agent_id:
        ...
        if not health_tracker.is_available(ag.id):
            raise HTTPException(
                409, detail=f"agent {ag.name} is stopped (circuit breaker open)"
            )
    if body.tool_name:
        ...
        t = (
            (
                await db.execute(
                    select(Tool).where(
                        Tool.org_id == org_id, Tool.name == body.tool_name
                    )
                )
            )
            .scalars()
        ...
        if t and t.input_schema:
            req = t.input_schema.get("required") or []
            for r in req:
                if r not in (body.tool_args or {}):
                    raise HTTPException(
                        400, detail=f"tool {body.tool_name} missing required arg: {r}"
                    )
            try:
                import jsonschema

                jsonschema.validate(body.tool_args or {}, t.input_schema)
            except ImportError:
                pass
            except Exception as e:
                raise HTTPException(400, detail=f"tool args invalid: {e}")
    existing_nodes = (
        (
            await db.execute(
    ...
    if body.tool_name and not body.agent_id and not body.tool_name.strip():
        raise HTTPException(400, detail="tool_name required if agent not set")
    for dep in body.depends_on or []:
        ...
        if dep == "new":
            raise HTTPException(400, detail="self-dependency not allowed")
    node = WorkflowNode(
        workflow_id=wf_id,
        label=body.label,
...
async def run_workflow(
    ...
    async_mode = body.get("async") if "async" in body else body.get("background", True)
    ...
    if async_mode:
        try:
            ...
            await enqueue_job(
                ...
                {"workflow_id": wf_id, "run_id": run.id},
            )
            run.status = "running"
            await db.commit()
    ...
    try:
        ...
        if result.errors:
            run.error = str(result.errors)[:4000]
        await db.commit()
        return {
            "run_id": run.id,
            "status": run.status,
...
async def get_run(
    ...
    if not run or run.workflow_id != wf_id or run.org_id != org_id:
        raise HTTPException(404)
    return run
...
async def list_runs(
    ...
    res = await db.execute(
        select(WorkflowRun)
        .where(WorkflowRun.workflow_id == wf_id, WorkflowRun.org_id == org_id)
...
async def eval_workflow_run(
    ...
    scored = await rubric_manager.score_response(
        rubric_id, output[:4000], llm_provider=llm
    )
...
@router.post("/{wf_id}/resume/{run_id}")
async def resume_run(
    ...
    if not orig or orig.workflow_id != wf_id or orig.org_id != org_id:
        raise HTTPException(404)
    if orig.status == "done":
        return {"run_id": orig.id, "status": "done", "outputs": orig.outputs}
    from aios.core.workflow import WorkflowDef, WorkflowNode as WNode, WorkflowEngine
    from sqlalchemy.orm import selectinload
    ...
    res = await eng.run(
        wdef,
        orig.conversation_id or orig.id,
        (orig.inputs or {}).get("input", ""),
        resume_outputs=dict(orig.outputs or {}),
        resume_status=dict(orig.node_status or {}),
    )
    orig.status = "done" if res.ok() else "failed"
    orig.outputs = res.outputs
    orig.node_status = res.node_status
    if res.errors:
...
async def replay_run(
    ...
    try:
        ...
        await enqueue_job(
            "aios.tasks.jobs.workflow_run_job",
            {"workflow_id": wf_id, "run_id": new_run.id},
        )
    except Exception:
        ...
        res = await eng.run(
            wdef,
            new_run.conversation_id or new_run.id,
            (new_run.inputs or {}).get("input", ""),
        )
